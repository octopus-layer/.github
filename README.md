...
Octopus creates a decentralized and censorship resistant verification layer for ZK snark proof verification that settles in Near and distributes ZK proofs to all blockchains through Near Multichain.
...
- Instead of every node signing and settling on Near one by one, it is enough only one node to send a settlement TX on Near with _66% of signatures_.

- Nevertheless, nodes do not compete to settle the proof, as all nodes that provided a signature during the settlement are _rewarded equally_ once the settlement is complete. This makes the need of a consensus layer inside Octopus dissapear.

- As it is for the best of all verifier nodes to settle as fast as possible, they try reaching each other in the most optimized manner. This creates a _perfect game theory_ and makes sure the settlement process will be complete in the smallest time possible.

- Moreover, nodes do not need to store anything. The URL of other nodes is provided from the Near contract. In a way, Near contract acts as a _communication layer_ between the verifier nodes of Octopus. As a result, there is _no synchronization_ during the installation of a new Octopus verifier node.
...
`Application Contract` is application spesific and differs from project to project. If an application on top of Near wants to use the Octopus Layer, their `Application Contract` must follow a spesific pattern (as in the example provided with this project).
...
- As this is a proof of concept project, the incentivization of `Verifier Node`s is not implemented. It should be added as decribed in the architecture.
...
- The joining process of a new node may be modeled better in terms of tokenomics to make sure the incentives keep the set at a spesific size: The set should be big enough to always provide trustlessness, but also should be as small as possible to decrease the proof settlement time to its minimum.
